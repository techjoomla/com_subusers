 ...
 * @since  1.6
 */
class SubusersModelOrganizations extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__tjsu_organizations` As a');

		// $query->join('LEFT', '#__tjsu_users AS uc ON uc.client_id = a.id');
		...
		$search = $app->getUserStateFromRequest('.filter.search', 'filter_organisation_search');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.name LIKE ' . $search . '  OR  a.created_by LIKE ' . $search . ' )');
			}
		}
