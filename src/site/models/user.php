 ...
 * @since  1.6
 */
class SubusersModelUser extends JModelItem
{
	/**
	...
	public function publish($id, $state)
	{
		...
		if ($id)
		{
			...
			// Create object to fetch user info
			$subQuery	= $db->getQuery(true);
			$subsubquery 		= $db->getQuery(true);

			if ($subQuery)
			{
				// Sub query to select user id and client id from tjsu_user table
				$subQuery->select('user_id,client_id')
						->from($db->quoteName('#__tjsu_users'))
						->where($db->quoteName('id') . " = " . $db->quote($tjsuAutoIncId));
			...
			if ($subsubquery)
			{
				// Subsubquery to select grp id and client id from es_group_xref table
				$subsubquery->select('grp_id')
						->from($db->quoteName('#__es_group_xref'))
						->where($db->quoteName('client_id') . " = " . $db->quote($cId));
				$db->setQuery($subsubquery);
				$clusterId = $db->loadResult();
			}

			$fields = array($db->quoteName('state') . ' = ' . $db->quote($state));

			// Uid = user_id is common in both tale
			$conditions = array($db->quoteName('uid') . ' = ' . $db->quote($uId),
								$db->quoteName('cluster_id') . ' = ' . $db->quote($clusterId)
							);

			// Main query to update record into social_clusters_nodes
