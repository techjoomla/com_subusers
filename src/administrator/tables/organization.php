...
/**
 * organization Table class
 *
 * @since  1.6
 */
class SubusersTableorganization extends JTable
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__tjsu_organizations', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_subusers.organization.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_subusers/access.xml',
				"/access/section[@name='organization']/"
			);
			$default_actions = JAccess::getAssetRules('com_subusers.organization.' . $array['id'])->getData();
			$array_jaccess   = array();
		...
		if (!empty($files['logo']))
		{
			...
			foreach ($files['logo'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/images/com_subusers/partners/' . $filename;
	...
	protected function _getAssetName()
	{
		...
		return 'com_subusers.organization.' . (int) $this->$k;
	}
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/images/com_subusers/partners/' . $logoFile);
			}
		}
