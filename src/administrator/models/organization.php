 ...
 * @since  1.6
 */
class SubusersModelOrganization extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_subusers.organization', 'organization',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	/**
	 * Method to duplicate an Organization
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__tjsu_organizations');
				$max             = $db->loadResult();
